...
export class NetflyClient {
  ...
  // Example method retained for compatibility with existing tests
  ...
  async sendDocument(xml: string): Promise<ApiResult> {
  ...
  // GET /documentsList
  async documentsList(params: { startDate: string; endDate: string; flow: DocumentFlow }): Promise<DocumentsListItem[]> {
  ...
  async receiveDocument(docId: number, flow: DocumentFlow): Promise<string> {
  ...
  async createParticipant(participant: Participant): Promise<ApiResult> {
  ...
  async updateParticipant(participant: Participant): Promise<ParticipantUpdateResult> {
    const url = this.buildUrl('/participantManagement')
  ...
  async deleteParticipant(id: number): Promise<ParticipantDeleteResult> {
  ...
  async participantsList(filters?: Partial<Pick<Participant, 'name' | 'contactName' | 'contactEmail' | 'participantId'>>): Promise<Participant[]> {
  ...
  async participantLookup(scheme: string, participant_id: string): Promise<ParticipantLookupResult> {
    const url = this.buildUrl('/participantLookup', { scheme, participant_id })
  ...
  // GET /webhook
  async getWebhook(): Promise<WebhookConfigResponse> {
    const url = this.buildUrl('/webhook')
  ...
  async validate(vesid: string, xml: string): Promise<Record<string, unknown>> {
